...
Nearby Lunch Extension is a self-hosted Chrome extension. It does not provide a developer-operated backend, shared API key, analytics service, advertising service, or account system.

## Location and Places requests

The extension requests browser location only after the user chooses to search for lunch. The location is sent over HTTPS from the extension to the user's own local backend at `https://localhost:3000`. That backend sends a Nearby Search request to Google Maps Platform using the user's own Google Places API key.
...
Google Maps Platform processes the Nearby Search request. Google Maps links and attribution are displayed with Places content. Google may process information under its own policies.

## Chrome Web Store Limited Use

The use of information received from Google APIs will adhere to the Chrome Web Store User Data Policy, including the Limited Use requirements. Location and the local client identifier are used only to provide nearby lunch recommendations, rate-limit the local backend, and prevent abuse. They are not sold, used for advertising, or used for unrelated profiling.
